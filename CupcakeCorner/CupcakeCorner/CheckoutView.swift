...
struct CheckoutView: View {
        ...
        GeometryReader { geo in
            ScrollView {
                VStack {
                    ...
                    Text("Your order cost is $ \(orderWrap.order.cost, specifier: "%.2f")")
                        .font(.title)
    ...
    func placeOrder() {
        // Encode Data
        guard let encoded = try? JSONEncoder().encode(orderWrap.order) else {
...
struct CheckoutView_Previews: PreviewProvider {
    static var previews: some View {
        CheckoutView(orderWrap: OrderWrapper())
    }
}
